...
Beyond those, the postings are double-entry and the book proves it --- every
transaction sums to zero per currency before anything is appended, and a trial
balance cancels on every day --- but a ledger \term{core} in production is
wider than that, and the rest is absent by choice rather than by oversight.
There is no \term{transaction} record grouping postings into a journal entry
with its own identity, only the event id they share; the chart is the minimum
that lets each event balance, with no account lifecycle (open, closed, frozen,
dormant) and no posting-rule templates, so what debits what is written into
handlers rather than configured; only two balance types exist where production
tracks booked, available, cleared, blocked and pending; there is no accounting
period, closing entry or statement generation; no reconciliation against an
external clearing file and no ageing of the suspense position the force post
creates; no dispute or chargeback lifecycle, so everything after settlement is
out of scope; no limits or velocity checks beyond the available-balance test;
and nothing is published outbound, so no downstream system can learn that any
of this happened.

Each of those is a day of work rather than a rethink, which is why they are
cuts and not risks --- with one exception. The transaction record is the one I
designed \term{for}: the commit path buffers postings, validates them as a set
and appends them together, so adding an identity to that unit is an addition
rather than a migration. The others would each need a decision I was not given
enough to make.
