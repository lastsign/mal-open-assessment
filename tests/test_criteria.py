...
def test_c3_the_day4_settlement_of_auth_a_is_accepted(book) -> None:
    ...
    settlement = next(
        e for e in book.entries if e.event_id == "E5" and e.account == "ACC-001"
    )
...
def test_c2_is_false_e7_does_not_cause_exactly_one_fee_on_day2(book, pit) -> None:
    ...
    retro_fees = [
        e.value_date for e in book.entries
        if e.kind == "FEE" and e.account in CUSTOMERS
    ]
    pit_fees = [
        e.value_date for e in pit.entries
        if e.kind == "FEE" and e.account in CUSTOMERS
    ]
...
def test_c4_is_false_an_unmatched_settlement_is_force_posted(book) -> None:
    ...
    posting = next(
        e for e in book.entries if e.event_id == "E6" and e.account == "ACC-001"
    )
...
def test_c6_is_false_nothing_returns_to_its_pre_e7_value(book) -> None:
    ...
    assert [e for e in without_e7.entries if e.kind == "FEE"] == []
    assert len([
        e for e in book.entries if e.kind == "FEE" and e.account in CUSTOMERS
    ]) == 3
...
def test_c7_is_false_three_instalments_of_3334_overpay_by_two_fils(book) -> None:
    ...
    parts = [e for e in book.entries if e.event_id == "E10" and e.account == "ACC-002"]
