...
from ledger.replay import CUSTOMERS, replay, stream
...
def test_c1_day2_restated_at_end_of_day5_is_minus_370(book) -> None:
    ...
    raw = Money.zero(AED)
    for transaction, entry in postings(book, account="ACC-001"):
...
def test_c3_the_day4_settlement_of_auth_a_is_accepted(book) -> None:
    ...
    transaction, settlement = next(
        iter(postings(book, event_id="E5", account="ACC-001"))
    )
    assert transaction.kind == "SETTLEMENT"
...
def test_c2_is_false_e7_does_not_cause_exactly_one_fee_on_day2(book, pit) -> None:
    ...
    retro_fees = [t.value_date for t, _ in customer_legs(book, kind="FEE")]
    pit_fees = [t.value_date for t, _ in customer_legs(pit, kind="FEE")]
...
def test_c4_is_false_an_unmatched_settlement_is_force_posted(book) -> None:
    ...
    _, posting = next(iter(postings(book, event_id="E6", account="ACC-001")))
...
def test_c6_is_false_nothing_returns_to_its_pre_e7_value(book) -> None:
    ...
    assert customer_legs(without_e7, kind="FEE") == []
...
def test_c7_is_false_three_instalments_of_3334_overpay_by_two_fils(book) -> None:
    ...
    parts = [e for _, e in postings(book, event_id="E10", account="ACC-002")]
...
def test_c8_is_false_the_rounding_remainder_is_placed_not_discarded(book) -> None:
    ...
    _, credit = next(iter(customer_legs(book, kind="INTEREST", account="ACC-001")))
